...
{
    public class AddTests : AppControllerTestsBase
    {
        ...
        {
            // Set up
            var msg = GetAddMsgList("1");

            // Act
            await _service.Add(Db, _poll.Id, msg);
        ...
        {
            // Set up
            var msg = GetAddMsgList("1", TestFromTime, TestBlockLength);
        ...
        {
            // Set up
            var msg = GetAddMsgList("abc");
            msg[0].TimeZone = new string('a', TimeLogEntry.MaxEntryTextLength + 1);

            // Act
            Func<Task> action = async () => await _service.Add(Db, _poll.Id, msg);
        ...
        {
            // Set up
            var msg = GetAddMsgList(new string('a', TimeLogEntry.MaxTimeZoneLength + 1));
        ...
        {
            // Set up
            var msg = GetAddMsgList(entryText);
        ...
        public void If_TimeBlockLength_is_not_positive_should_throw(double minutes)
        {
            // Set up
            var msg = GetAddMsgList("1");
            msg[0].TimeBlockLength = TimeSpan.FromMinutes(minutes);
        ...
        public void If_TimeBlockLength_is_longer_than_allowed_should_throw()
        {
            // Set up
            var msg = GetAddMsgList("1");
            msg[0].TimeBlockLength = TimeLogService.MaxAllowedTimeBlockLength + TimeSpan.FromSeconds(1);
        ...
        public async void Should_force_fromTime_to_align_to_start_of_minute(string msgTime, string savedTime)
        {
            // Set up
            var msg = GetAddMsgList("1");
            msg[0].TimeCollected = DateTime.Parse(msgTime);
        ...
        private List<PollResponseAddRequest> GetAddMsgList(string entryText, DateTime fromTime = default, TimeSpan timeBlockLength = default)
        {
            return new List<PollResponseAddRequest>() { GetAddMsg(entryText, fromTime, timeBlockLength) };
        }
    }
}
