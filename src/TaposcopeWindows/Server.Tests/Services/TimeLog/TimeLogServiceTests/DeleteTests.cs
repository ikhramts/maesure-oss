...
namespace Server.Tests.Services.TimeLog.TimeLogServiceTests
{
    public class DeleteTests : AppControllerTestsBase
    {
        ...
        [Fact]
        public void If_Deletions_is_null_should_throw()
        {
            // Set up
            var req = new TimeLogDeleteRequest();

            // Act
            Func<Task> action = async () => await _service.Delete(Db, _poll.Id, req);

            // Assert
            action.Should().Throw<BadRequestException>().WithMessage("*deletions*");
        }
        ...
        public async void Should_insert_deletion_entry_for_every_range()
        {
            ...
            // Act
            await _service.Delete(Db, _poll.Id, req);
            ...
            entry0.CreatedTimeUtc.Should().Be(TestUtcNow);
            ...
            entry1.CreatedTimeUtc.Should().Be(TestUtcNow);
        }
